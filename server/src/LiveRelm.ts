...
export class LiveRelm extends Observable<string> {
  ...
  getOccupancyMap() {
    ...
    for (const portalName of this.portals.values()) {
  ...
  join(participantId: string) {
    ...
    console.log(`'${this.relmName}' now has ${this.occupancy} ` + `participants ('${participantId}' joined)`)
  }

  leave(participantId: string) {
    this.participantIds.delete(participantId)
    console.log(`'${this.relmName}' now has ${this.occupancy} ` + `participants ('${participantId}' left)`)
  }
}

export const liveRelms = new Map<string, LiveRelm>()

export function getOrCreateLiveRelm(relmDoc: RelmDocWithName, awareness?: Awareness) {
  if (!liveRelms.has(relmDoc.relmId))
    liveRelms.set(relmDoc.relmId, new LiveRelm(relmDoc.relmId, relmDoc.relmName, awareness))
