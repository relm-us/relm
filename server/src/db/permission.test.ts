...
describe("Permission model tests", () => {
  ...
  it("filteredPermits", () => {
    expect(Permission.filteredPermits(["access", "xyz"])).toEqual(new Set(["access"]))
    expect(Permission.filteredPermits(["admin", "access", "invite", "edit"])).toEqual(
      new Set(["admin", "access", "invite", "edit"]),
    )
  })
  ...
  it("getPermissions returns multiple permits including wildcard relm", async () => {
    ...
    expect(Object.keys(permitsByRelm).sort()).toEqual(["*", relmName1, relmName2].sort())
  ...
  it("sets permissions", async () => {
    ...
    for (const permit of ["access", "invite", "edit"]) {
      await Permission.setPermits({
        participantId,
        relmId: relmId1,
  ...
  it("sets unioned permissions", async () => {
    ...
    for (const permit of ["access", "invite", "invite"]) {
      await Permission.setPermits({
        participantId,
        relmId: relmId1,
        permits: [permit as Permission.Permission],
    ...
    expect(permitsByRelm[relmName1].sort()).toEqual(["access", "invite"].sort())
  })
