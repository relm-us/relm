...
relm.post(
  ...
  wrapAsync(async (req, res) => {
    ...
    if (seedRelmId) {
      ...
      if (!seedRelm) {
        throw Error(`relm can't be created because seed relm named '${seedRelmName}' doesn't exist`)
      }
      seedRelmId = seedRelm.relmId
    }
...

    {
      ...
      if (seedRelmId) {
        ...
        console.log(`Created relm '${req.relmName}' ` + `(creator: '${req.authenticatedParticipantId}')`)
      }
...
async function setVariable(doc, variables, relmId, name, value) {
  ...
  if (!possibilities) {
    ...
    console.warn(`Unable to load possible actions for relm ${relmId};` + ` not setting variable ${name}`)
    return
  }
...
function setProperties(doc: Y.Doc, entityId: string, componentsToSet: EntityComponentData): ActionResultSetProperties {
  // Ensure components passed are valid
