...
asset.post(
  ...
  wrapAsync(async (req, res) => {
    if (!req.body.entityId) return respondWithError(res, "entityId is required")
  ...
  wrapAsync(async (req, res) => {
    ...
    try {
      switch (extension) {
        ...
        case ".wav":
        case ".webm": {
          const sound = await conversion.moveOrUploadContentAddressable(asset.tempFilePath, extension)
        ...
        case ".webp": {
          ...
          const png = await conversion.moveOrUploadContentAddressable(pngTempFile)

          const webpTempFile = `${asset.tempFilePath}.webp`
          await sharp(asset.tempFilePath).toFile(`${asset.tempFilePath}.webp`)
          const webp = await conversion.moveOrUploadContentAddressable(webpTempFile)
        ...
        case ".gltf":
        case ".packed-gltf": {
          const gltf = await conversion.moveOrUploadContentAddressable(asset.tempFilePath, extension)
