...
import { decodedValidJwt, hasPermission, joinError, respondWithError, unabbreviatedPermits } from "./utils/index.js"
...
export function relmExists() {
  return async (req, res, next) => {
    try {
      console.log("relmExists", req.relmName)
      ...
      if (!req.relm) {
        ...
        if (config.UNSAFE_AUTOINIT && req.relmName === "default" && isFirstUser) {
          // The first user gets special admin permissions for all relms, including default
...
export function acceptJwt() {
  return async (req, _res, next) => {
    if (config.JWTSECRET && req.headers["x-relm-jwt"]) {
      const result: { relms: Record<string, string> } = decodedValidJwt(req.headers["x-relm-jwt"], config.JWTSECRET)
      if (result) {
        req.jwtRaw = result
...
export function authorized(requestedPermission: Permission.Permission | ((req: any) => Permission.Permission)) {
  return async (req, _res, next) => {
